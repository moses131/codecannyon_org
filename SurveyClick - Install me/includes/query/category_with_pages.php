...
class category_with_pages extends \util\db {
    ...
    private $count          = false;
    // db query
    protected $select       = 'p.*, cp.category as cp_category';
    protected $selectKey;
    ...
    public function setCategoryId( int $id ) {
        $this->conditions['category'] = [ 'cp.category', '=', $id ];
        return $this;
    }

    public function setPageId( int $id ) {
        $this->conditions['page'] = [ 'cp.page', '=', $id ];
        return $this;
    }

    public function excludePageId( int $id ) {
        $this->conditions['exclude_' . $id] = [ 'cp.page', '!=', $id ];
        return $this;
    }

    public function excludeCategoryId( int $id ) {
        $this->conditions['exclude_' . $id] = [ 'cp.category', '!=', $id ];
        return $this;
    }
    ...
    private function orderBy_values() {
        $list                   = [];
        $list['id']             = 'p.id';
        $list['id_desc']        = 'p.id DESC';
        $list['title']          = 'p.title';
        $list['title_desc']     = 'p.title DESC';
        $list['date']           = 'p.date';
        $list['date_desc']      = 'p.date DESC';
    ...
    public function count() {
        ...
        $query .= $this->table( 'category_pages cp' );
        $query .= ' LEFT JOIN ';
        $query .= $this->table( 'pages p' );
        $query .= ' ON p.id = cp.page';
    ...
    public function fetch( int $max = 0, int $offset = 0 ) {
        ...
        while( ( $row = $result->fetch_assoc() ) ) {
            if( $this->selectKey )
                $data[$row[$this->selectKey]] = $this->filters->do_filter( 'category_with_pages_info_values', (object) $row );
            else
                $data[] = $this->filters->do_filter( 'category_with_pages_info_values', (object) $row );
        }
