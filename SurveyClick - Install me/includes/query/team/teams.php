...
class teams extends \util\db {
    ...
    function __construct( int $id = 0, int $user = 0 ) {
        parent::__construct();

        $this->setId( $id );
        $this->setUser( $user );
        $this->orderby  = $this->filters->do_filter( 'teams_default_order_by', [ 'id_desc' ] );
    }
    ...
    public function search( string $name ) {
        if( $name !== '' ) {
            ...
            filters()->add_filter( 'teams_order_by_values', function( $f, $list ) {
    ...
    public function getLastChatMessageUserId() {
        return $this->info->lcm_user;
    }

    public function getLastChatMessage() {
        return $this->info->lcm;
    }
    ...
    public function isOwner() {
        return ( !empty( $this->info ) && $this->user == $this->info->user );
    }
    ...
    public function limits() {
        ...
        $this->limits = new \query\user_limits( $this->info->user );
        return $this->limits;
    }
    ...
    private function orderBy_values() {
        ...
        return $this->filters->do_filter( 'teams_order_by_values', $list );
    }
    ...
    public function itemsPerPage() {
        return $this->filters->do_filter( 'teams_per_page', $this->items_per_page );
    }
    ...
    public function count() {
        if( ( $count = $this->filters->do_filter( 'teams_set_count', $this->count ) ) !== false ) {
            return $count;
        }
        ...
        if( $count > 0 ) return $this->filters->do_filter( 'teams_count', $count );
    ...
    public function info( int $id = 0 ) {
        ...
        if( $fields ) {
            return $this->filters->do_filter( 'teams_info_values', $fields );
        }
    ...
    public function fetch( int $max = 0, int $offset = 0 ) {
        ...
        while( ( $row = $result->fetch_assoc() ) ) {
            if( $this->selectKey )
                $data[$row[$this->selectKey]] = $this->filters->do_filter( 'teams_info_values', (object) $row );
            else
                $data[] = $this->filters->do_filter( 'teams_info_values', (object) $row );
        }
    ...
    public function userIsMember( int $user ) {
        $query = 'SELECT * FROM ';
        ...
        if( $fields ) {
            return $this->filters->do_filter( 'teams_membership_info_values', $fields );
        }
