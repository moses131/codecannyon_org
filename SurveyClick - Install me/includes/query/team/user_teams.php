...
class user_teams extends \util\db {
    ...
    private $count          = false;
    // db query
    protected $select       = 't.*, tm.id as tm_id, tm.approved as tm_approved, tm.date as tm_date';
    ...
    function __construct( int $id = 0 ) {
        ...
        $this->setId( $id );
        $this->orderby  = $this->filters->do_filter( 'user_teams_default_order_by', [ 'id_desc' ] );
    }
    ...
    public function setUserId( int $id ) {
        $this->conditions['user'] = [ 'tm.user', '=', $id ];
        return $this;
    }
    ...
    private function orderBy_values() {
        ...
        return $this->filters->do_filter( 'user_teams_order_by_values', $list );
    }
    ...
    public function itemsPerPage() {
        return $this->filters->do_filter( 'user_teams_per_page', $this->items_per_page );
    }

    public function count() {
        if( ( $count = $this->filters->do_filter( 'user_teams_set_count', $this->count ) ) !== false ) {
            return $count;
        }
    ...
    public function fetch( int $max = 0, int $offset = 0 ) {
        ...
        $query .= $this->table( 'teams_members tm' );
        $query .= ' LEFT JOIN ';
        $query .= $this->table( 'teams t' );
        $query .= ' ON (tm.team = t.id) ';
        $query .= $this->finalCondition();
        ...
        while( ( $row = $result->fetch_assoc() ) ) {
            if( $this->selectKey )
                $data[$row[$this->selectKey]] = $this->filters->do_filter( 'user_teams_info_values', (object) $row );
            else
                $data[] = $this->filters->do_filter( 'user_teams_info_values', (object) $row );
        }
