...
class category_pages extends \util\db {
    ...
    public function itemsPerPage() {
        return $this->filters->do_filter( 'category_pages_per_page', $this->items_per_page );
    }
    ...
    public function count() {
        if( ( $count = $this->filters->do_filter( 'category_pages_set_count', $this->count ) ) !== false ) {
            return $count;
        }

        $query = 'SELECT COUNT(*) FROM ';
        $query .= $this->table( 'category_pages' );
        ...
        if( $count > 0 ) return $this->filters->do_filter( 'category_pages_count', $count );
    ...
    public function info( int $id = 0 ) {
        ...
        if( $fields ) {
            return $this->filters->do_filter( 'category_pages_info_values', $fields );
        }
    ...
    public function fetch( int $max = 0, int $offset = 0 ) {
        ...
        while( ( $row = $result->fetch_assoc() ) ) {
            if( $this->selectKey )
                $data[$row[$this->selectKey]] = $this->filters->do_filter( 'category_pages_info_values', (object) $row );
            else
                $data[] = $this->filters->do_filter( 'category_pages_info_values', (object) $row );
        }
