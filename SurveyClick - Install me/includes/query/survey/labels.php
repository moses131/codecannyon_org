...
class labels extends \util\db {
    ...
    function __construct( int $id = 0 ) {
        ...
        $this->setId( $id );
        $this->orderby  = $this->filters->do_filter( 'labels_default_order_by', [ 'name' ] );
    }
    ...
    private function orderBy_values() {
        ...
        return $this->filters->do_filter( 'labels_order_by_values', $list );
    }
    ...
    public function itemsPerPage() {
        return $this->filters->do_filter( 'labels_per_page', $this->items_per_page );
    }
    ...
    public function count() {
        if( ( $count = $this->filters->do_filter( 'labels_set_count', $this->count ) ) !== false ) {
            return $count;
        }

        $query = 'SELECT COUNT(*) FROM ';
        $query .= $this->table( 'labels' ); 
        ...
        if( $count > 0 ) return $this->filters->do_filter( 'labels_count', $count );
    ...
    public function info( int $id = 0 ) {
        ...
        if( $fields ) {
            return $this->filters->do_filter( 'labels_info_values', $fields );
        }
    ...
    public function fetch( int $max = 0, bool $pagination = true ) {
        ...
        while( ( $row = $result->fetch_assoc() ) ) {
            if( $this->selectKey )
                $data[$row[$this->selectKey]] = $this->filters->do_filter( 'labels_info_values', (object) $row );
            else
                $data[] = $this->filters->do_filter( 'labels_info_values', (object) $row );
        }
