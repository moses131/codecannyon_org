...
class invoices extends \util\db {
    ...
    function __construct( int $id = 0 ) {
        ...
        $this->setId( $id );
        $this->orderby  = $this->filters->do_filter( 'invoices_default_order_by', [ 'id_desc' ] );
    }
    ...
    public function setNumberId( int $id ) {
        $this->conditions['number'] = [ 'number', '=', $id ];
        return $this;
    }
    ...
    public function search( string $name ) {
        $this->conditions['search'] = [ 'number', 'LIKE', '%'. $this->dbp( $name ) . '%' ];
        return $this;
    }
    ...
    public function getBillTo() {
        return $this->info->bill_to;
    }

    public function getBillToJson() {
        return ( $this->info->bill_to ? json_decode( $this->info->bill_to, true ) : [] );
    }
    ...
    public function getTaxesF() {
        return cms_money_format( $this->info->tax );
    }
    ...
    public function getSubtotalF() {
        return cms_money_format( $this->info->subtotal );
    }
    ...
    public function getTotalF() {
        return cms_money_format( $this->info->total );
    }
    ...
    private function orderBy_values() {
        ...
        return $this->filters->do_filter( 'invoices_order_by_values', $list );
    }
    ...
    public function itemsPerPage() {
        return $this->filters->do_filter( 'invoices_per_page', $this->items_per_page );
    }
    ...
    public function count() {
        if( ( $count = $this->filters->do_filter( 'invoices_set_count', $this->count ) ) !== false ) {
            return $count;
        }
        ...
        if( $count > 0 ) return $this->filters->do_filter( 'invoices_count', $count );
    ...
    public function info( int $id = 0 ) {
        ...
        if( $fields ) {
            return $this->filters->do_filter( 'invoices_info_values', $fields );
        }
    ...
    public function fetch( int $max = 0, int $offset = 0 ) {
        ...
        while( ( $row = $result->fetch_assoc() ) ) {
            if( $this->selectKey )
                $data[$row[$this->selectKey]] = $this->filters->do_filter( 'invoices_info_values', (object) $row );
            else
                $data[] = $this->filters->do_filter( 'invoices_info_values', (object) $row );
        }
