...
class actions extends \util\db {
    ...
    function __construct( int $id = 0 ) {
        ...
        $this->setId( $id );
        $this->orderby  = $this->filters->do_filter( 'admin_actions_default_order_by', [ 'id_desc' ] );
    }
    ...
    public function setByUserId( int $id ) {
        $this->conditions['by_user'] = [ 'by_user', '=', $id ];
        return $this;
    }

    public function setToUserId( int $id ) {
        $this->conditions['to_user'] = [ 'to_user', '=', $id ];
        return $this;
    }
    ...
    public function getByUserId() {
        return $this->info->by_user;
    }

    public function getToUserId() {
        return $this->info->to_user;
    }
    ...
    public function getByUser() {
        if( !isset( $this->info->by_user ) ) return false;
        $users  = new users;
        $users  ->setId( $this->info->by_user );
        return $users;
    }

    public function getToUser() {
        if( !isset( $this->info->to_user ) ) return false;
        $users  = new users;
        $users  ->setId( $this->info->to_user );
        return $users;
    }
    ...
    private function orderBy_values() {
        ...
        return $this->filters->do_filter( 'admin_actions_order_by_values', $list );
    }
    ...
    public function itemsPerPage() {
        return $this->filters->do_filter( 'admin_actions_per_page', $this->items_per_page );
    }
    ...
    public function count() {
        if( ( $count = $this->filters->do_filter( 'admin_actions_set_count', $this->count ) ) !== false ) {
            return $count;
        }
        ...
        if( $count > 0 ) return $this->filters->do_filter( 'admin_actions_count', $count );
    ...
    public function info( int $id = 0 ) {
        ...
        if( $fields ) {
            return $this->filters->do_filter( 'admin_actions_info_values', $fields );
        }
    ...
    public function fetch( int $max = 0, int $offset = 0 ) {
        ...
        while( ( $row = $result->fetch_assoc() ) ) {
            if( $this->selectKey )
                $data[$row[$this->selectKey]] = $this->filters->do_filter( 'admin_actions_info_values', (object) $row );
            else
                $data[] = $this->filters->do_filter( 'admin_actions_info_values', (object) $row );
        }
