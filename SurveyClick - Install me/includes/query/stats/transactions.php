...
class transactions extends \util\db {
    ...
    public function generateReport( array $from, array $to, bool $saveReport = false, string $groupBy = 'day' ) {
        $query = 'WITH t AS (SELECT amount, DATE_FORMAT(date, ?) as date, date as o_date FROM ';
        $query .= $this->table( 'transactions' ); 
        $query .= ' WHERE type ' . ( is_array( $this->type ) ? 'IN(' . array_map( 'intval', $this->type ) . ')' : '= ' . (int) $this->type );

        if( $this->survey ) {
            $query .= ' AND survey = ' . (int) $this->survey;
        }

        if( $this->user ) {
            $query .= ' AND user = ' . (int) $this->user;
        }

        if( $this->status ) {
            $query .= ' AND `status` = ' . (int) $this->status;
        }

        $query .= ' AND date >= ? AND date <= ?) SELECT SUM(t.amount) as sum, COUNT(*) as total, t.o_date as o_date, t.date as `date` FROM t GROUP BY t.date';
        ...
        while( ( $row = $result->fetch_assoc() ) ) {
            $data[date( $this->date_format, strtotime( $row['o_date'] ) )]  = $this->filters->do_filter( 'transactions_stats_info_values', (object) $row );
        }
