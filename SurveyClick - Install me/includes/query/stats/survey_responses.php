...
class survey_responses extends \util\db {
    ...
    public function generateReport( array $from, array $to, bool $saveReport = false, string $groupBy = 'day' ) {
        ...
        $query  = 'WITH t AS (SELECT DATE_FORMAT(r.date, ?) as date, r.date as o_date FROM ';
        ...
        while( ( $row = $result->fetch_assoc() ) ) {
            $data[date( $this->date_format, strtotime( $row['o_date'] ) )]   = $this->filters->do_filter( 'survey_responses_stats_info_values', (object) $row );
        }
