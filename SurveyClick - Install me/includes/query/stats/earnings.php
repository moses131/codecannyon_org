...
class earnings extends \util\db {

    private $survey;
    private $user;
    private $collector;
    private $status;
    private $include_commissions;
    private $last_report    = [];
    ...
    public function setIncludeCommissions( bool $show = true ) {
        $this->include_commissions = $show;
        return $this;
    }
    ...
    public function autoFillDates() {
       ...
       foreach( $period as $date ) {
            ...
            $dates[$date]   = isset( $this->last_report[$date] ) ? $this->last_report[$date] : (object) [ 'sum' => 0, 'responses_done' => 0, 'date' => $date ];
        }
    ...
    public function generateReport( array $from, array $to, bool $saveReport = false, string $groupBy = 'day' ) {
        ...
        if( $this->include_commissions )
        $query  = 'WITH t AS (SELECT (commission + commission_bonus) as e_commission, DATE_FORMAT(date, ?) as date, date as o_date FROM ';
        else
        $query  = 'WITH t AS (SELECT (commission + commission_bonus - commission_p) as e_commission, DATE_FORMAT(date, ?) as date, date as o_date FROM ';
        ...
        $query .= 'date >= ? AND date <= ?) SELECT SUM(t.e_commission) as sum, COUNT(*) as responses_done, t.o_date as o_date, t.date as `date` FROM t GROUP BY t.date';
        ...
        while( ( $row = $result->fetch_assoc() ) ) {
            $data[date( $this->date_format, strtotime( $row['o_date'] ) )]   = $this->filters->do_filter( 'earnings_stats_info_values', (object) $row );
        }
